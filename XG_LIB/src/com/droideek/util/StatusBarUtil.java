...
public class StatusBarUtil {
    ...
    public static boolean setStatusBarDarkIcon(Activity activity, boolean dark) {
        boolean result = false;
        String m = BaseApplication.B_MANUFACTUER;
        if(m.indexOf("meizu")>=0) {
            return setMeizuStatusBarDarkIcon(activity, dark);
        }else if(m.indexOf("xiaomi")>=0) {
            return setMiuiStatusBarDarkMode(activity, dark);
        }
     ...
     * @param alpha
     */
    public static void setColor(Activity activity, int lightColorRes, int darkColorRes, int alpha) {
        if (setStatusBarDarkIcon(activity, true)) {
            StatusBarUtil.setColor(activity, lightColorRes, alpha);
        }else{
           // StatusBarUtil.setColor(activity, darkColorRes, alpha);
        }
    }
}
