...
import com.lib.pulltorefresh.library.internal.FlipLoadingLayout;
import com.lib.pulltorefresh.library.internal.LoadingLayout;
import com.lib.pulltorefresh.library.internal.Utils;
import com.lib.pulltorefresh.library.internal.ViewCompat;
import com.lib.pulltorefresh.library.seagoor.XGLoadingDogLayout;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	final void setState(State state, final boolean... params) {
		mState = state;
		if (DEBUG) {
			Log.d(LOG_TAG, "setState(), State: " + mState.name());
		}
	...
	protected final int getHeaderSize() {
		if (DEBUG) {
			Log.d(LOG_TAG, "getHeaderSize();" + mHeaderLayout.getContentSize());
		}
		...
		if (doScroll) {
			if (mShowViewWhileRefreshing) {
				...
				switch (mCurrentMode) {
					...
					case PULL_FROM_START:
						if (DEBUG) {
							Log.d(LOG_TAG, "==onRefreshing(); smoothScrollTo:" + (-getHeaderSize()));
						}
				...
				if (DEBUG) {
					Log.d(LOG_TAG, "==onRefreshing(); smoothScrollTo: 0");
				}
				smoothScrollTo(0);
			}
		} else {
	...
	protected int getRefreshHeight() {
		int h = mHeaderLayout.getRefreshHeight();
		if (h <= 0) {
	...
	private void pullEvent() {
		...
		switch (mCurrentMode) {
			case PULL_FROM_END:
				...
				itemDimension = getRefreshHeight();
				break;
			case PULL_FROM_START:
			default:
		...
		if (newScrollValue != 0 && !isRefreshing()) {
			...
			switch (mCurrentMode) {
				...
				default:
					if (DEBUG) {
						Log.d(LOG_TAG, "==pullEvent(); scale:"+scale);
					}
	...
	public static enum AnimationStyle {
		...
		LoadingLayout createLoadingLayout(Context context, Mode mode, Orientation scrollDirection, TypedArray attrs) {
			switch (this) {
				...
				default:
				case SEAGOOR:
					return new XGLoadingDogLayout(context, mode, scrollDirection, attrs);
	...
	final class SmoothScrollRunnable implements Runnable {
		...
		public void run() {
			...
			if (mStartTime == -1) {
				...
				if (DEBUG) {
					Log.d(LOG_TAG, "RUNNABLE; mCurrentY:"+mCurrentY + " mScrollFromY:"+ mScrollFromY + " mScrollToY"+mScrollToY);
				}
