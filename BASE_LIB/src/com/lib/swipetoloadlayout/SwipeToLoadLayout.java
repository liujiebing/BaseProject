package com.lib.swipetoloadlayout;
...
import com.lib.swipetoloadlayout.seagoor.Logr;
...
public class SwipeToLoadLayout extends ViewGroup {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (STATUS.isSwipingToRefresh(mStatus) || STATUS.isReleaseToRefresh(mStatus)
                    ...
                    mAutoScroller.abortIfRunning();
                    return false; //UPDATE SOURCE FROM "return true";
                }
     ...
     * @param refreshing
     */
    public void setRefreshing(boolean refreshing, boolean scrollHeader) {
        Logr.d(TAG, "== setRefreshing(); refreshing:%b scrollHeader: %b", refreshing, scrollHeader);
        ...
        if (refreshing) {
            if (STATUS.isStatusDefault(mStatus)) {
                setStatus(STATUS.STATUS_SWIPING_TO_REFRESH);
                if(scrollHeader) scrollDefaultToRefreshing();
            }
        } else {
