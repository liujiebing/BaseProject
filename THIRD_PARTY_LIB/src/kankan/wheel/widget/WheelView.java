...
public class WheelView extends View {
    ...
    /**
     * 设定其中一项的高度， 用于需要多个WHELL——view 时中间线条需要对齐的情况
     */
    ...
    public void detach() {
        ...
        if (null != changingListeners && changingListeners.size() > 0) {
            changingListeners.clear();
        }
        if (null != scrollingListeners && scrollingListeners.size() > 0) {
            scrollingListeners.clear();
        }

        if (null != clickingListeners && clickingListeners.size() > 0) {
            clickingListeners.clear();
        }

        if (null != topShadow) {
            topShadow.setCallback(null);
        }
        if (null != bottomShadow) {
            bottomShadow.setCallback(null);
        }
